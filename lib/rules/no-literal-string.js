...
const defauleConfig = {
  ...
  useCallee: 'lang.t',
  i18nFilePath: path.resolve(process.cwd(), './src/locales/google-translate')
};
 ...
 */
async function appendToJSONFile(filePath, key, value) {
  try {
      let data = {};
...
function autoTranslate(zhCnText, i18nFilePath) {
  translateText(zhCnText).then((translatedText) => {
    // 输出 translatedText 作为翻译结果
    // 格式为 zhCnText: translatedText
    // 找到默认的配置文件
    const filePath = path.resolve(i18nFilePath, './zh-CN.json');
    // 写入到文件中
  ...
  create(context) {
    ...
    function validateLiteralNode(node) {
      ...
      context.report({
        ...
        message,
        fix(fixer) {
          autoTranslate(node.value, i18nFilePath);
